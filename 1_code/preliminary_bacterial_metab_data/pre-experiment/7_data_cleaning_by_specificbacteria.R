...
Enterococcus_faecium_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Enterococcus faecium")) %>%
  pull(sample_id)
Blautia_producta_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Blautia producta")) %>%
  pull(sample_id)
Parabacteroides_merdae_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Parabacteroides merdae")) %>%
  pull(sample_id)
Bifidobacterium_faecale_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Bifidobacterium faecale")) %>%
  pull(sample_id)
Bacteroides_fragilis_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Bacteroides fragilis")) %>%
  pull(sample_id)
Parabacteroides_distasonis_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Parabacteroides distasonis")) %>%
  pull(sample_id)
Anaerotignum_lactatifermentans_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Anaerotignum lactatifermentans")) %>%
  pull(sample_id)
Blautia_faecis_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Blautia faecis")) %>%
  pull(sample_id)
Bifidobacterium_pseudocatenulatum_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Bifidobacterium pseudocatenulatum")) %>%
  pull(sample_id)
Enterococcus_faecalis_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Enterococcus faecalis")) %>%
  pull(sample_id)
Bacteroides_vulgatus_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(Bacteria %in% c("Bacteroides vulgatus")) %>%
  pull(sample_id)
zeroh_medium_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(sample_id %in% c("POS  0h-1 (2).", "POS  0h-2 (2).", "POS  0h-3 (2).", "POS  0h-4 (2).")) %>%
  pull(sample_id)
sixh_medium_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(sample_id %in% c("POS  6h-1 (2).", "POS  6h-2 (2).", "POS  6h-3 (2).", "POS  6h-4 (2).")) %>%
  pull(sample_id)
tenh_medium_id <- object_pos %>%
  activate_mass_dataset(what = "sample_info") %>%
  filter(sample_id %in% c("POS  10h-1 (2).", "POS  10h-2 (2).", "POS  10h-3 (2).")) %>%
  pull(sample_id)

object_pos =
  object_pos %>%
  mutate_variable_na_freq(according_to_samples = Enterococcus_faecium_id) %>%
  mutate_variable_na_freq(according_to_samples = Blautia_producta_id) %>%
  mutate_variable_na_freq(according_to_samples = Parabacteroides_merdae_id) %>%
  mutate_variable_na_freq(according_to_samples = Bifidobacterium_faecale_id) %>%
  mutate_variable_na_freq(according_to_samples = Bacteroides_fragilis_id) %>%
  mutate_variable_na_freq(according_to_samples = Parabacteroides_distasonis_id) %>%
  mutate_variable_na_freq(according_to_samples = Anaerotignum_lactatifermentans_id) %>%
  mutate_variable_na_freq(according_to_samples = Blautia_faecis_id) %>%
  mutate_variable_na_freq(according_to_samples = Bifidobacterium_pseudocatenulatum_id) %>%
  mutate_variable_na_freq(according_to_samples = Enterococcus_faecalis_id) %>%
  mutate_variable_na_freq(according_to_samples = Bacteroides_vulgatus_id) %>%
  mutate_variable_na_freq(according_to_samples = zeroh_medium_id) %>%
  mutate_variable_na_freq(according_to_samples = sixh_medium_id) %>%
  mutate_variable_na_freq(according_to_samples = tenh_medium_id)  
