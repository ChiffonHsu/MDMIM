...
# 2. Filter MetaCyc edges touching your nodes
metacyc_sub_edges <- metacyc_edges_filtered_unique %>%
  filter(grepl("^InChI=1S/", from_inchi) & grepl("^InChI=1S/", to_inchi)) %>%
  filter(from_inchi %in% ecoli_nodes$INCHI_ID | to_inchi %in% ecoli_nodes$INCHI_ID)

# 3. Filter KEGG edges touching your nodes
kegg_sub_edges <- kegg_edge_final %>%
  mutate(from_inchi = INCHI_From,
         to_inchi   = INCHI_To) %>%
  filter(from_inchi %in% ecoli_nodes$INCHI_ID |
           to_inchi   %in% ecoli_nodes$INCHI_ID)

# 4. Combine both datasets
colnames(metacyc_sub_edges)
colnames(kegg_sub_edges)
kegg_sub_edges <- kegg_sub_edges %>%
  dplyr::select(from_inchi, to_inchi, Source)
combined_edges <- bind_rows(
  metacyc_sub_edges %>%
    dplyr::select(from_inchi, to_inchi) %>%
    mutate(Source = "MetaCyc"),
...
# 5. Detect down -> up reaction pairs
reaction_pairs_detected <- combined_edges %>%
  filter(from_inchi %in% down_nodes$INCHI_ID & to_inchi %in% up_nodes$INCHI_ID)
...
reaction_pairs_named <- reaction_pairs_detected %>%
  ...
  dplyr::select(from_label, to_label, from_inchi, to_inchi, Source)
...
all_sub_edges <- combined_edges %>%
  filter(from_inchi %in% ecoli_nodes$INCHI_ID | to_inchi %in% ecoli_nodes$INCHI_ID) %>%
  dplyr::left_join(
    ecoli_nodes %>% select(INCHI_ID, Compound.name) %>% rename(from_name = Compound.name),
    by = c("from_inchi" = "INCHI_ID")
  ) %>%
  dplyr::left_join(
    ecoli_nodes %>% select(INCHI_ID, Compound.name) %>% rename(to_name = Compound.name),
    by = c("to_inchi" = "INCHI_ID")
  ) %>%
  dplyr::mutate(
    from_label = ifelse(!is.na(from_name), from_name, from_inchi),
