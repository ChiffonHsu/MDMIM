...
# [1] "INCHI_Reaction_ID"  "INCHI_From"         "INCHI_To"           "Source_From"        "Source_To"         
...
#   )
nrow(kegg_edge_final) #3778
sum(ecoli_nodes$INCHI_ID %in% kegg_edge_final$INCHI_From |
      ecoli_nodes$INCHI_ID %in% kegg_edge_final$INCHI_To) #25
# keep only edges where either side is in your ecoli_nodes list
kegg_sub_edges <- kegg_edge_final %>%
  filter(
    INCHI_From %in% ecoli_nodes$INCHI_ID |
      INCHI_To %in% ecoli_nodes$INCHI_ID
  )
nrow(kegg_sub_edges) #45
# collect unique InChIs involved in those reactions
sub_inchis <- unique(c(kegg_sub_edges$INCHI_From, kegg_sub_edges$INCHI_To))

length(sub_inchis) #95
# Join with your ecoli_nodes to add readable names
reaction_pairs_named <- kegg_sub_edges %>%
  left_join(
    ecoli_nodes %>%
      dplyr::select(INCHI_ID, Compound.name) %>%
      dplyr::rename(from_name = Compound.name),
    by = c("INCHI_From" = "INCHI_ID")
  ) %>%
  left_join(
    ecoli_nodes %>%
      dplyr::select(INCHI_ID, Compound.name) %>%
      dplyr::rename(to_name = Compound.name),
    by = c("INCHI_To" = "INCHI_ID")
  ) %>%
  mutate(
    from_label = ifelse(!is.na(from_name), from_name, INCHI_From),
    to_label   = ifelse(!is.na(to_name), to_name, INCHI_To)
  ) %>%
  dplyr::select(from_label, to_label)


#according to up and down
# 1. Separate nodes into up and down
up_nodes <- ecoli_nodes %>% filter(direction == "up")
down_nodes <- ecoli_nodes %>% filter(direction == "down")

# 2. Filter KEGG sub-edges to keep only exact "from → to" pairs
kegg_reaction_pairs_detected <- kegg_sub_edges %>%
  filter(
    INCHI_From %in% down_nodes$INCHI_ID &  # down node is the source
      INCHI_To   %in% up_nodes$INCHI_ID      # up node is the target
  )

# 3. Add readable names for easier inspection
kegg_reaction_pairs_named <- kegg_reaction_pairs_detected %>%
  left_join(
    ecoli_nodes %>% dplyr::select(INCHI_ID, Compound.name) %>% dplyr::rename(from_name = Compound.name),
    by = c("INCHI_From" = "INCHI_ID")
  ) %>%
  left_join(
    ecoli_nodes %>% dplyr::select(INCHI_ID, Compound.name) %>% dplyr::rename(to_name = Compound.name),
    by = c("INCHI_To" = "INCHI_ID")
  ) %>%
  mutate(
    from_label = ifelse(!is.na(from_name), from_name, INCHI_From),
  ...
  dplyr::select(from_label, to_label, INCHI_From, INCHI_To, INCHI_Reaction_ID)

# 4. Check results
nrow(kegg_reaction_pairs_named) #0
kegg_reaction_pairs_named #0

#mix the previous two together
# 1. Separate nodes
up_nodes <- ecoli_nodes %>% filter(direction == "up")
down_nodes <- ecoli_nodes %>% filter(direction == "down")

# 2. Filter edges for down -> up reaction pairs
reaction_pairs_detected <- kegg_edge_final %>%
  filter(INCHI_From %in% down_nodes$INCHI_ID &
           INCHI_To   %in% up_nodes$INCHI_ID)

# 3. Annotate with readable names
reaction_pairs_named <- reaction_pairs_detected %>%
  ...
  dplyr::select(from_label, to_label, INCHI_From, INCHI_To, INCHI_Reaction_ID, Source)
